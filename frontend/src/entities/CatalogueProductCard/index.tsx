...
export const CatalogueProductCard = (product: IProduct) => {
    const {id, image, price, title, discount, isFavourite} = product;
    ...
    return (
        <ProductContainerStyled withImageAnimation={Boolean(image)}>
            ...
            <TopSideBoxLinkToProductStyled to={String(id)}>
                <ProductImageStyled id="image" src={image || WithoutImage} />
            </TopSideBoxLinkToProductStyled>
            <div>
                <ProductTitleStyled to={String(id)} id="title">{title}</ProductTitleStyled>
                <ProductPriceBlockStyled>
                ...
                <AddToCardButtonStyled id="cartBtn" onClick={() => setProductToCart(product)}>
                    В корзину
                </AddToCardButtonStyled>
            </div>
        </ProductContainerStyled>
